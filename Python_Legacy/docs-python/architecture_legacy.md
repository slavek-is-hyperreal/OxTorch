# Architecture: OxTorch Legacy Engine (Python + Taichi)
...
The original OxTorch engine (`import vulkan_nn_lib`) is a Python-first implementation designed for maximum flexibility.
...
For LLM inference in the Python version, OxTorch uses a `BlockTable` to map token sequences to scattered physical blocks in VRAM. This eliminates fragmentation but incurs Python GIL overhead during virtualization.
...
- **Workflow**: 
  1. Local OxTorch tiles the operation.
