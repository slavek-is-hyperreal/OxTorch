...
# Konfiguracja
TARGET_DIR = "/my_data/gaussian_room/deep_research_prompts"
PRECISION = "FP32"
OPERATIONS = [
    "Add", "Sub", "Mul", "Div", "Pow", "Minimum", "Maximum", "Atan2"
]

MASTER_PROMPT = """### TASK: World-Class High-Performance Kernel Research for OxTorch/VNN
...
- IMPORTANT: Review techniques used in scientific libraries like **BLIS**, **libxsmm**, **FFTW**, and open-source physics simulators (e.g., GROMACS, LAMMPS) for {operation} in {precision}.
...
def generate():
    ...
    for op in OPERATIONS:
        filename = f"prompt_{op.lower()}_{PRECISION.lower()}.md"
        filepath = os.path.join(TARGET_DIR, filename)
        
        content = MASTER_PROMPT.format(operation=op, precision=PRECISION)
        
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content)
        
        print(f"Wygenerowano (World-Class Version): {filename}")

if __name__ == "__main__":
    generate()
    print("\nGotowe! Folder deep_research_prompts zawiera teraz specyfikacje dla 'World-Leading' kernels.")
