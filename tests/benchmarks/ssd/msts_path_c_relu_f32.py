"""
MSTS Path C (Full Multi-worker) — ReLU F32 SSD
Tensor size: >= 32 MB.
2 IO workers + Rayon parallel compute. Full CrookScheduler.
...
Benchmark fairness note:
  ...
  At this size (~256MB), the I/O bottleneck dominates completely —
  this is the true break-even test for io_uring vs VFS throughput.
"""
import os, sys, time
import numpy as np
...
# Path C: size >= 32 MB → 16M f32 elements = ~64 MB read + ~64 MB write = ~128 MB total I/O
N_ELEMENTS = 16_000_000
ITERS = 2
NAME = "MSTS_PathC_ReLU_F32_SSD"
SSD_IN     = "/tmp/msts_bench_c.ssd"
SSD_OUT    = "/tmp/msts_bench_c_relu.ssd"
SSD_PT_OUT = "/tmp/msts_bench_c_pt_relu.bin"
...
def run():
    ...
    # ── Parity (first 1M elements only) ─────────────────────────────────────
    result_data = np.array(result_tensor.load_to_f32_vec_msts())
    expected = np.maximum(data, 0.0)
    check = min(1_000_000, len(result_data))
    max_diff = float(np.max(np.abs(result_data[:check] - expected[:check])))
    parity = max_diff < 1e-4

    ratio = t_vnn / t_pt if t_pt > 0 else 0.0
    # Total I/O: read input + write output (both files)
    total_io_mb = (N_ELEMENTS * 4 * 2) / 1e6
    mb_per_s_vnn = total_io_mb / t_vnn
    mb_per_s_pt  = total_io_mb / t_pt

    print(f"\n>>> TEST: {NAME} | SSD Path C (Multi-worker) | N={N_ELEMENTS:,} | Iter: {ITERS}")
    print(f"    Comparing: disk read→relu→disk write (~{total_io_mb:.0f} MB total I/O each)")
    print(f"    [PyTorch/VFS]  {t_pt:.4f}s  ({mb_per_s_pt:.1f} MB/s total I/O)")
    print(f"    [OxTorch/uring]{t_vnn:.4f}s  ({mb_per_s_vnn:.1f} MB/s total I/O) | Ratio: {ratio:.2f}x | Parity: {'✅ PASS' if parity else '❌ FAIL'} (max_diff={max_diff:.2e})")
