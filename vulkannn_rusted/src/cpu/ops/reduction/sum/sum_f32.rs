...
unsafe fn sum_f32_avx512(buf: &[f32]) -> f32 {
    ...
    let mut s = tmp.iter().map(|&x| x as f64).sum::<f64>(); for &x in &buf[n16..] { s += x as f64; } s as f32
}
...
unsafe fn sum_f32_avx(buf: &[f32]) -> f32 {
    ...
    let mut s = tmp.iter().map(|&x| x as f64).sum::<f64>(); for &x in &buf[n8..] { s += x as f64; } s as f32
}
