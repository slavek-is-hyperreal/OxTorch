//! cat_f32 — concatenate f32 tensors along `dim`. MOVE-not-rewrite from
//! cpu_old/ops/sequence/cat/f32.rs: pure memory movement (copy_from_slice of
//! contiguous stride-`dim` chunks), no numeric accumulation, bit-exact. Special
//! fast path for dim==0 (fully contiguous append); general case walks the outer
//! dims and copies each tensor's dim-slab into place.
use crate::tensor::{Tensor, DataType, Storage};

pub fn cat_f32(tensors: &[&Tensor], dim: usize) -> Vec<f32> {
    debug_assert!(tensors[0].dtype == DataType::F32);
