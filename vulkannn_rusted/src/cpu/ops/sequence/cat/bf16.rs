//! cat_bf16 — MOVE-not-rewrite from cpu_old (pure memory movement, bit-exact).
use crate::tensor::{Tensor, DataType, Storage};
pub fn cat_bf16(tensors: &[&Tensor], dim: usize) -> Vec<half::bf16> {
    debug_assert!(tensors[0].dtype == DataType::BF16);
    ...
    let mut out_data = vec![half::bf16::ZERO; total_size];
    ...
    for (i, t) in tensors.iter().enumerate() { dim_offsets[i] = current_offset; current_offset += t.shape[dim]; }
    for outer_idx in 0..outer_size {
        for (i, t) in tensors.iter().enumerate() {
            let t_dim_size = t.shape[dim];
            let t_stride_dim = t_dim_size * inner_size;
            if let Storage::BF16(v) = &t.storage {
