...
pub fn layer_norm_f16(x: &[half::f16], w: &[half::f16], b: &[half::f16], out: &mut [half::f16], n: usize, d: usize, eps: f32) {
    if n > 1 {
        x.par_chunks(d).zip(out.par_chunks_mut(d)).for_each(|(x_row, out_row)| {
            layer_norm_f16_row(x_row, w, b, out_row, d, eps);
        });
    } else {
        layer_norm_f16_row(x, w, b, out, d, eps);
    }
}

fn layer_norm_f16_row(x: &[half::f16], w: &[half::f16], b: &[half::f16], out: &mut [half::f16], d: usize, eps: f32) {
