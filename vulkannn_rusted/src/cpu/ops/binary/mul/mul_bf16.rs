...
/// Elementwise multiplication for BF16 tensors with multi-architecture support.
pub fn mul_bf16(a: &[half::bf16], b: &[half::bf16], res: &mut [half::bf16]) {
    ...
    if a.len() > PAR_THRESHOLD {
        ...
        a.chunks(PAR_THRESHOLD).zip(b.chunks(PAR_THRESHOLD)).zip(res.chunks_mut(PAR_THRESHOLD)).par_bridge().for_each(|((ac, bc), rc)| {
            mul_bf16_serial(ac, bc, rc);
        });
    } else {
        mul_bf16_serial(a, b, res);
    }
}

fn mul_bf16_serial(a: &[half::bf16], b: &[half::bf16], res: &mut [half::bf16]) {
    #[cfg(target_arch = "x86_64")] {
        if is_x86_feature_detected!("avx") {
            return unsafe { mul_bf16_avx(a, b, res) };
        }
    }

    #[cfg(target_arch = "aarch64")] {
        return mul_bf16_neon(a, b, res);
    }

    mul_bf16_scalar(a, b, res);
}

#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx")]
unsafe fn mul_bf16_avx(a: &[half::bf16], b: &[half::bf16], res: &mut [half::bf16]) {
    let n8 = (a.len() / 8) * 8;
    ...
    mul_bf16_scalar(&a[n8..], &b[n8..], &mut res[n8..]);
}

#[cfg(target_arch = "aarch64")]
fn mul_bf16_neon(a: &[half::bf16], b: &[half::bf16], res: &mut [half::bf16]) {
    ...
    mul_bf16_scalar(&a[n4..], &b[n4..], &mut res[n4..]);
}

fn mul_bf16_scalar(a: &[half::bf16], b: &[half::bf16], res: &mut [half::bf16]) {
    for i in 0..a.len() {
