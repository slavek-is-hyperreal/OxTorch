...
/// Serial AVX implementation for BF16 addition.
/// Optimized as a "Leaf Kernel": serial SIMD loop without internal threading.
/// Handles 8 elements per iteration (256-bit registers).
#[target_feature(enable = "avx")]
pub unsafe fn add_bf16_avx_serial(a: &[half::bf16], b: &[half::bf16], res: &mut [half::bf16]) {
    let n = a.len();
    let n8 = (n / 8) * 8;
    ...
    for i in (0..n8).step_by(8) {
        ...
        // Upcast to 8 F32 (256 bits) by shifting BF16 bits to the high 16 bits of F32
        ...
        // Compute addition in F32
        let vr = _mm256_add_ps(va, vb);
        
        // Downcast back to BF16 (Truncation)
    ...
    // Generic scalar fallback for tail elements
    if n > n8 {
        for i in n8..n {
