//! SSE2 (no-AVX tier) Implementation for I8 Sub (saturating, `_mm_subs_epi8`).
//! Transcribed from cpu_old/ops/binary/sub/sub_i8.rs (legacy sub_i8_sse2, which
//! legacy reached via its `swar` slot on x86). Correct hardware saturating sub.
//!
...
#[cfg(target_arch = "x86_64")]
pub unsafe fn sub_i8_sse2(a: &[i8], b: &[i8], res: &mut [i8]) {
    let n = a.len();
    let n16 = (n / 16) * 16;
