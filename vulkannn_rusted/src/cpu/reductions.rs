...
pub fn sum_i8_dispatch(buf: &[i8]) -> i32 {
    const PAR_LIMIT: usize = 256_000;
    if buf.len() > PAR_LIMIT { return buf.par_chunks(PAR_LIMIT).map(|c| sum_i8_serial(c)).sum(); }
    sum_i8_serial(buf)
}

fn sum_i8_serial(buf: &[i8]) -> i32 {
    #[cfg(target_arch = "x86_64")] {
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx512f")]
unsafe fn sum_i8_avx512(buf: &[i8]) -> i32 {
    ...
    for i in (0..n64).step_by(64) {
        ...
        // Pack I8 to I16 then I32 for summation
    ...
    let mut s_acc = s; for &b in &buf[n64..] { s_acc += b as i32; } s_acc
}

#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn sum_i8_avx2(buf: &[i8]) -> i32 {
    ...
    let mut s = tmp.iter().sum::<i32>(); for &b in &buf[n32..] { s += b as i32; } s
}

#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx")]
unsafe fn sum_i8_avx1(buf: &[i8]) -> i32 {
    ...
    let mut s = tmp.iter().sum::<i32>(); for &b in &buf[n16..] { s += b as i32; } s
}

pub fn sum_i8_swar(buf: &[i8]) -> i32 {
    buf.iter().map(|&x| x as i32).sum()
}
