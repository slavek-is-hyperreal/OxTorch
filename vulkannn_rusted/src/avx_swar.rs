...
unsafe fn convert_f32_to_bf16_avx2(src: &[f32], dst: &mut [bf16]) {
    ...
    for (s, d) in chunks.zip(dst_chunks.by_ref()) {
        ...
        // Shift right to get the upper 16 bits
        let shifted = _mm256_srli_epi32(vec_i, 16);
        // Pack 32-bit into 16-bit
        ...
        let mut tmp = [0u16; 8];
        _mm_storeu_si64(tmp.as_mut_ptr() as *mut u8, low);
        _mm_storeu_si64(tmp[4..].as_mut_ptr() as *mut u8, high);
        for i in 0..8 { d[i] = bf16::from_bits(tmp[i]); }
    }
...
unsafe fn convert_f32_to_bf16_sse2(src: &[f32], dst: &mut [bf16]) {
    ...
    for (s, d) in chunks.zip(dst_chunks.by_ref()) {
        ...
        let mut tmp = [0u16; 4];
        _mm_storel_epi64(tmp.as_mut_ptr() as *mut __m128i, packed);
