    ...
    if is_x86_feature_detected!("avx2") {
        for chunk in buf.chunks_exact_mut(8) {
            unsafe {
                ...
                let res = crate::cpu_old::ops::math_simd::silu_ps_avx2(v);
    ...
    #[cfg(target_arch = "aarch64")]
    {
        for chunk in buf.chunks_exact_mut(4) {
            unsafe {
                ...
                let res = crate::cpu_old::ops::math_simd::silu_ps_neon(v);
...
pub fn silu_i8(buf: &mut [i8]) {
    // SiLU LUT for int8
    static mut SILU_LUT: [i8; 256] = [0; 256];
    static INIT: std::sync::Once = std::sync::Once::new();
    INIT.call_once(|| {
        for i in 0..256 {
            let x = (i as i32 - 128) as f32 / 16.0;
            let res = x / (1.0 + (-x).exp());
            unsafe { SILU_LUT[i] = (res * 16.0).round() as i8; }
        }
    });
    for x in buf.iter_mut() {
        *x = unsafe { SILU_LUT[(*x as i32 + 128) as usize] };
    }
}
