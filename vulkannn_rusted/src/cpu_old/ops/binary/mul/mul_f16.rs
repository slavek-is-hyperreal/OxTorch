...
/// Elementwise multiplication for F16 tensors with multi-architecture support.
pub fn mul_f16(a: &[half::f16], b: &[half::f16], res: &mut [half::f16]) {
    ...
    if a.len() > PAR_THRESHOLD {
        ...
        a.chunks(PAR_THRESHOLD).zip(b.chunks(PAR_THRESHOLD)).zip(res.chunks_mut(PAR_THRESHOLD)).par_bridge().for_each(|((ac, bc), rc)| {
            mul_f16_serial(ac, bc, rc);
        });
    } else {
        mul_f16_serial(a, b, res);
    }
}

fn mul_f16_serial(a: &[half::f16], b: &[half::f16], res: &mut [half::f16]) {
    #[cfg(target_arch = "x86_64")] {
        if is_x86_feature_detected!("f16c") && is_x86_feature_detected!("avx") {
            return unsafe { mul_f16_f16c(a, b, res) };
        }
    }

    #[cfg(target_arch = "aarch64")] {
        return mul_f16_neon(a, b, res);
    }

    mul_f16_scalar(a, b, res);
}

#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx,f16c")]
unsafe fn mul_f16_f16c(a: &[half::f16], b: &[half::f16], res: &mut [half::f16]) {
    let n8 = (a.len() / 8) * 8;
    ...
    mul_f16_scalar(&a[n8..], &b[n8..], &mut res[n8..]);
}

#[cfg(target_arch = "aarch64")]
fn mul_f16_neon(a: &[half::f16], b: &[half::f16], res: &mut [half::f16]) {
...
fn mul_f16_scalar(a: &[half::f16], b: &[half::f16], res: &mut [half::f16]) {
    for i in 0..a.len() {
