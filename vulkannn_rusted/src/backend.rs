                    ...
                    loop {
                        ...
                        unsafe {
                            matrixmultiply::sgemm(
                                ...
                                b_ptr as *const f32, n as isize, 1, 0.0,
            ...
            loop {
                ...
                for n_start in (0..n).step_by(block_n as usize) {
                    ...
                    for (t_idx, &k_start) in k_tiles.iter().enumerate() {
                        ...
                        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                            label: Some("MatMul Bindings"), layout: &pipeline.get_bind_group_layout(0),
                            entries: &[
                    ...
                    let [a0, a1] = bufs_a;
                    let [b0, b1] = bufs_b;
                    recycle_buffer(a0, (bm * block_k * 4) as u64, usage_storage); recycle_buffer(a1, (bm * block_k * 4) as u64, usage_storage);
                    recycle_buffer(b0, (block_k * bn * 4) as u64, usage_storage); recycle_buffer(b1, (block_k * bn * 4) as u64, usage_storage);
                    ...
                    res_band.par_chunks_mut(n as usize).enumerate().for_each(|(i, row)| {
...
pub fn execute_activation_into(input_data: &[f32], op: &str, result: &mut [f32], is_hybrid: bool, use_staging: bool) {
    let backend = BACKEND.get().expect("Backend not initialized");
    let device = &backend.device;
    let queue = &backend.queue;
    ...
    std::thread::scope(|s| {
        ...
        if gpu_elements > 0 {
            ...
            let chunk_elements = 8 * 1024 * 1024;
            let num_chunks = (gpu_elements + chunk_elements - 1) / chunk_elements;
            const NUM_STAGES: usize = 3;
            ...
            for i in 0..NUM_STAGES {
                buf_i.push(get_buffer(device, size, usage_storage, Some(&format!("Act I {}", i))));
                buf_o.push(get_buffer(device, size, usage_result, Some(&format!("Act O {}", i))));
                staging_bufs.push(get_buffer(device, size, usage_staging, Some(&format!("Staging {}", i))));
            }
            ...
            for chunk_idx in 0..num_chunks {
                ...
                if use_staging {
                    let mut tmp_i = vec![0.0; current_elements];
                    tmp_i.copy_from_slice(&i_gpu[chunk_start..chunk_start + current_elements]);
                    queue.write_buffer(&buf_i[stage], 0, bytemuck::cast_slice(&tmp_i));
                } else {
